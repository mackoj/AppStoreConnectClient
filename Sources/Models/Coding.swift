...
extension KeyedDecodingContainer {
  ...
  func decodeAny<T>(_ type: T.Type, forKey key: K) throws -> T {
    guard let value = try decode(AnyCodable.self, forKey: key).value as? T else {
      throw DecodingError.typeMismatch(
        T.self,
        DecodingError.Context(
          codingPath: codingPath, debugDescription: "Decoding of \(T.self) failed"))
    }
    return value
  }
  ...
  public func decodeIfPresent<T>(_ key: KeyedDecodingContainer.Key) throws -> T?
  where T: Decodable {
    return try decodeOptional {
...
public let dateDecoder: (Decoder) throws -> Date = { decoder in
  ...
  guard
    let date = formatterWithMilliseconds.date(from: string)
